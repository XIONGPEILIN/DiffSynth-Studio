...
class QwenImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.units = [
                ...
                QwenImageUnit_MaskGuidedNoise(), # 新增：mask 引导的 bbox noise 替换
        ...
        self.ste = STE()
        # Ensure STE weights match pipeline device/dtype to avoid mismatches at runtime
    ...
    def training_loss(self, **inputs):
        ...
        inputs["sub_latents"] = self.scheduler.add_noise(inputs["sub_input_latents"], sub_noise, timestep)
        sub_training_target = self.scheduler.training_target(inputs["sub_input_latents"], sub_noise, timestep)


        noise_pred , sub_noise_pred = self.model_fn(**inputs, timestep=timestep)

        back_loss = torch.nn.functional.mse_loss(noise_pred.float(), training_target.float())
        sub_loss = torch.nn.functional.mse_loss(sub_noise_pred.float(), sub_training_target.float())
        loss = (back_loss + sub_loss)/2
        org_loss = loss.clone()
